...
export default async function handler(req, res) {
  ...
  try {
    ...
    const results = await Promise.allSettled([
      ...
      // 8. Horas Pico
      db.execute(`SELECT CAST(SUBSTR(entry, 1, INSTR(entry || ':', ':') - 1) AS INTEGER) as hora, COUNT(*) as total FROM historial WHERE date >= date('now', '-7 days') AND entry IS NOT NULL AND entry != '' GROUP BY hora ORDER BY hora`),
      // 9. Historial Reciente
      db.execute(`SELECT * FROM historial ORDER BY id DESC LIMIT 8`),
      // 10. Tipo de Vehículo (últimos 30 días en historial)
      db.execute(`
      ...
      db.execute(`
        ...
        LIMIT 5
      `),
      // 12. CORREGIDO: Desglose vehículos ADENTRO por tipo
      // La tabla puestos NO tiene columna 'plate'. 
      // Se usa cliente_id para hacer JOIN con clientes.tipo_vehiculo
      db.execute(`
        SELECT c.tipo_vehiculo as tipo, COUNT(*) as total 
        FROM puestos p 
    ...
    // 12. Desglose por tipo adentro (CORREGIDO)
    if (results[12].status === 'fulfilled') {
      results[12].value.rows.forEach(r => {
    ...
    if (results[5].status === 'fulfilled' && results[6].status === 'fulfilled') {
      const mapa = {};
      results[5].value.rows.forEach(r => { if(!mapa[r.date]) mapa[r.date] = { ingresos: 0, gastos: 0 }; mapa[r.date].ingresos += r.monto; });
      results[6].value.rows.forEach(r => { if(!mapa[r.date]) mapa[r.date] = { ingresos: 0, gastos: 0 }; mapa[r.date].gastos += r.monto; });
    ...
    if (results[7].status === 'fulfilled') {
      chartMetodosPagoData = results[7].value.rows.map(r => ({ metodo: r.method || 'Sin definir', total: r.total }));
    }

    // 8. Horas Pico
    if (results[8].status === 'fulfilled') {
      chartHorasPicoData = results[8].value.rows.filter(r => r.hora !== null && r.hora >= 0 && r.hora <= 23);
    }

    // 9. Movimientos Recientes
    if (results[9].status === 'fulfilled') {
      movimientosRecientes = results[9].value.rows;
    }

    // 10. Tipo de Vehículo
    if (results[10].status === 'fulfilled') {
      chartTipoVehiculoData = results[10].value.rows;
    }

    // 11. Top 5 Clientes
    if (results[11].status === 'fulfilled') {
      chartTopClientesData = results[11].value.rows;
    }
