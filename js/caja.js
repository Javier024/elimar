document.addEventListener("DOMContentLoaded", function () {
  ...
  let allTransactions = []
  let currentPage = 1
  const itemsPerPage = 5 // Paginación unificada

  // ==============================
  // CONFIGURACIÓN PARQUEADERO
  // ==============================
  const PARKING = {
  ...
  async function loadTransactions() {
    try {
      ...
      currentPage = 1 // Reiniciar a página 1 al cargar
      renderTable()
    } catch (error) {
...
`
  ...
  function renderTable() {
    ...
    // Calcular KPIs (Total global, no solo de la página)
    let totalIncome = 0
    let cashIncome = 0
    let cardIncome = 0
