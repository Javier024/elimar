document.addEventListener("DOMContentLoaded", function () {
  ...
  // ==============================
  // 1. FILTROS AVANZADOS (Fecha, Placa, Tipo)
  // ==============================
  window.applyFilters = function() {
    ...
    const plateStr = document.getElementById("filterPlate").value.toLowerCase();
  ...
  window.downloadReport = function() {
    // Leer qué quiere descargar el usuario
    const downloadType = document.getElementById("downloadType").value;
    
    // Filtrar los datos globales según la selección
    let dataToDownload = allHistory;
    if (downloadType !== "all") {
      dataToDownload = allHistory.filter(item => item.type === downloadType);
    }

    if (dataToDownload.length === 0) {
      mostrarToast("No hay datos de ese tipo para descargar", "error");
      return;
    }
  ...
  window.saveManualHistory = async function() {
    ...
    const spot = document.getElementById("manualSpot").value;
  ...
  function updatePagination() {
    ...
    controls.innerHTML = `
      ...
      <button onclick="changePage('next')" class="px-3 py-1 bg-white border border-slate-300 rounded text-xs font-bold ${currentPage === totalPages || totalPages === 0 ? 'opacity-50' : ''}" ${currentPage === totalPages || totalPages === 0 ? 'disabled' : ''}>Siguiente</button>
    `;
  }
