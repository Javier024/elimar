...
let totalSpent=0
let topExpense={amount:0,concept:"-"}
...
if(amount>topExpense.amount) topExpense=g
...
set("kpiTotal",formatMoney(totalSpent))
set("kpiTopConcept",topExpense.concept)
set("kpiTopAmount",formatMoney(topExpense.amount))
set("kpiCount",gastosData.length)
...
<td class="px-6 py-4 text-sm">${gasto.date}</td>

<td class="px-6 py-4 font-medium">${gasto.concept}</td>

<td class="px-6 py-4">
<span class="px-2 py-1 text-xs border rounded ${catColor}">
${gasto.category}
</span>
</td>

<td class="px-6 py-4 text-right font-bold text-red-500">
${formatMoney(gasto.amount)}
</td>
...
btn.className="px-2 py-1 text-xs border rounded mx-1"
...
alert("Ingrese concepto y monto válido")
return
